...
**Your backend is running on:** `localhost:8323` ✅
...
**Change the port from `8585` to `8323`:**
...
// AFTER (Correct):
const javaURI = "http://localhost:8323";
```
...
export const javaURI = "http://localhost:8323";  // ← Make sure it's 8323!
...
Edit `assets/js/api/config.js` and change `8585` → `8323`
...
```bash
curl http://localhost:8323/api/post/all
# Should return: []
```
...
| Service | Port | Status |
|---------|------|--------|
| **Flask Backend** | **8323** | ✅ Correct - Use this! |
...
1. **Backend is on 8323** - This is confirmed and working ✅
2. **Don't change backend** - It's correct!
3. **Change frontend config** - Update to port 8323
...
Once you update the port to `8323`:
...
```javascript
// Backend API Configuration
export const javaURI = "http://localhost:8323";
...
# Find and replace 8585 with 8323
find assets -name "*.js" -type f -exec sed -i '' 's/8585/8323/g' {} +
...
| What | Where | Change |
|------|-------|--------|
| **Backend** | Flask (running) | Port **8323** ✅ |
| **Frontend Config** | `config.js` | Change to **8323** ❌→✅ |
...
Just update that one line in your frontend config from `8585` to `8323` and everything will work!
